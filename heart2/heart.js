...
setInterval(function(_) { // The main draw loop. _ is unused; just here to match the other function sig (for jscrush savings)
  ...
  for(i=1696,z=w!=(w=Math.max(0,Math.min(1,.5-3*Math.sin(t/8))));i-=2;) {
    ...
    l=Math.sin(Z=t+3*o.pageY/H)*m[i].y+Math.cos(Z)*m[i][j], // Used in 3D rotation calc
            ...
            a.beginPath();
            a.arc( // Stick arc in the free semicolon zone
      ...
      Math.max(0,6-t),6);
            a.closePath();
            a.fill(), // Controls the fade in at the beginning
    ...
    i%41||1//a.fillText("❁✿♡❤"[3&t/4/Math.PI],W/2+Math.sin(t/8+i)*W/2,H/2+Math.cos(t/8+i*9)*Math.sin(t/2)*H/2)
  }
...
copy();}, w=16) // Aim for 16 fps, also define w (morph offset, value doesn't matter yet)
